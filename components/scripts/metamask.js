...
function toggleButton() {
  ...
  if (window.ethereum == undefined) {
    //if not installed then do the following.
...
async function loginWithMetamask() {
  ...
  try {
    //this will get the list of all the accounts in the metamask of that user...
    accounts = await window.ethereum.request({
      method: 'eth_requestAccounts',
    });
  } catch {
    //if there is no account, or user cancels the request.
    userWallet.innerText = 'Something Went Wrong';
    return;
  }

  if (!accounts) {
    //if there are no accounts return.
    return;
  }
